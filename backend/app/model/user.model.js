        ...
        if (err) {
            ...
            bcrypt.compare(body.password, data[0].password, (err, res)=> {
                if (err) {
                    ...
                    console.log(data);
                    console.log("login successfully occured..");
                    return callback(null, data);
                } else {
                    console.log("incorrect password please check.. ");
            ...
            console.log("username not in database please check it..")
...
usermodel.prototype.forgotPassword = (body, callback) => {
 ...
 usermodel.prototype.resetPassword = (body, callback) => {
     user.find({ 'email': body.email }, (err, data) => {
         if (err) {
             ...
             response = { "error": true, "message": "Email already exists in database ", "errorCode": 404 };
             return callback(response);
         }
         else {
             const newUser = new user({
 ...
 usermodel.prototype.getAllUser = (req,callback) => {
     user.find({}, (err, data)=>{
        if(err){
            callback("error is in user model" + err)
        }else{
            callback(null , data);
        }
     })
